...
- (CGFloat)_updateMagnifier:(YYTextMagnifier *)mag {
    ...
    if (mag.captureDisabled) {
        if (!mag.snapshot || mag.snapshot.size.width > 1) {
            ...
            dispatch_once(&onceToken, ^{
                ...
                placeholder = [render imageWithActions:^(UIGraphicsImageRendererContext * _Nonnull rendererContext) {
    ...
    UIGraphicsImageRenderer * render = [[UIGraphicsImageRenderer alloc] initWithSize:captureRect.size];
